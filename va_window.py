...
class Connect4(udebs.State):
    ...
    def result(self, alpha=None, beta=None):
        # Check for end (negamax doesn't handle this)
        ...
        # Set default alpha and beta args
        if beta is None:
            beta = int(map_.x * map_.y / 2) + 1
        ...
        while alpha < beta:
            med = int(alpha + (beta - alpha) / 2)
            if med <= 0 and alpha / 2 > med:
                med = int(alpha / 2)
            elif med >= 0 and beta/2 > med:
                med = int(beta / 2)

            v = self.negamax(med, med+1, map_)
